...
if (
    ...
    $ins_ang = "INSERT INTO angariacoes (data_insercao_ilist, freguesia_imovel, arruamento_imovel, num_porta_imovel, complemento_imovel, cod_postal_imovel, tipo_angariacao, tipo_negocio, tipo_imovel, valor_inicial_imovel) 
    VALUES ('$data_insercao_ilist', '$id_freg_imv', '$arruamento_imv', '$num_porta_imv', '$complemento_imv', '$cod_postal_imv', '$tipo_angariacao', '$tipo_negocio', '$tipo_imv', '$valor_inicial_imovel')";

    if ($conn->query($ins_ang) === TRUE) {
        $last_id_ang = $conn->insert_id;
        $ins_lvr = "INSERT INTO livro_registo (estado_imovel, num_contrato, data_cmi, regime, id_inicial_imovel, angariacao_id, nome_proprietario, nif_proprietario, email_proprietario, telefone_proprietario, arruamento_proprietario, num_porta_proprietario, complemento_proprietario, cod_postal_proprietario, freguesia_proprietario, conservatoria, ficha_crp, matriz_cpu, tipo_comissao, valor_comissao, data_negocio, data_recisao, valor_negocio, comissao_recebida) 
        VALUES ('$estado_imovel', '$num_contrato', '$data_cmi', '$regime', '$id_angariacao_inicial', $last_id_ang, '$nome_prop', '$nif_prop', '$email_prop', '$telefone_prop', '$arruamento_pro', '$num_porta_prop', '$complemento_prop', '$cod_postal_prop', '$id_freg_prop', '$conservatoria', '$ficha_crp', '$matriz_cpu', '$tipo_comissao', '$valor_comissao', NULL, NULL, NULL, NULL)";
        if ($conn->query($ins_lvr) === TRUE) {
            ...
            VALUES ('$data_insercao_ilist', '$last_id_ang', '$id_angariacao_inicial', '$id_consultor', '$id_gestor', '$loja_consultor', 0, 0, 0, 0)";
            ...
            if ($conn->query($ins_id_ang) === TRUE) {
                ...
                VALUES ('$data_insercao_ilist', '$last_id_ang', '$valor_inicial_imovel', '$valor_inicial_imovel', '$tipo_angariacao', '$tipo_negocio', NULL, '$by_user')";

                if ($conn->query($ins_baixa_preco) === TRUE) {
                    $conn->commit();
                    $_SESSION['msg_nova_angariacao'] = "<div class='note note-success font-green-haze'><h4 class='block bold'>✔ Angariação Inserida com Sucesso!</h4><p>Parabéns por ter inserido todos os campos corretamente.</p></div>";
                    header("Location: ../../coordenacao/nova-angariacao");
                } else {
                    $conn->rollback();
